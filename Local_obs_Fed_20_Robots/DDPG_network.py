...
class Federated_Server(object):
    def __init__(self, name_actor, name_critic, input_dims, n_actions=7, layer1_size=32, layer2_size=32):
        ...
        self.ServerFederation = [self.actor.params[i].assign((tf.multiply(self.actor_params0[i], 1) +
                                                              ...
                                                              tf.multiply(self.actor_params14[i], 1) +
                                                              tf.multiply(self.actor_params15[i], 1) +
                                                              tf.multiply(self.actor_params16[i], 1) +
                                                              tf.multiply(self.actor_params17[i], 1) +
                                                              tf.multiply(self.actor_params18[i], 1) +
                                                              tf.multiply(self.actor_params19[i], 1)) / 20) for i in range(len(self.actor.params))]
