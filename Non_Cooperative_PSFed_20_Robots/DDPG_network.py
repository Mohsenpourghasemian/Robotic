...
class Federated_Server(object):
    def __init__(self, name_actor, name_critic, input_dims, n_actions=4, layer1_size=32, layer2_size=32):
        self.sess = tf.Session()
        ...
        self.actor_params15 = self.actor.params
        self.actor_params16 = self.actor.params
        self.actor_params17 = self.actor.params
        self.actor_params18 = self.actor.params
        self.actor_params19 = self.actor.params
        self.Robot0_sent = 0
        self.Robot1_sent = 0
        self.Robot2_sent = 0
        self.Robot3_sent = 0
        ...
        self.ServerFederation = [self.actor.params[i].assign((tf.multiply(self.actor_params0[i], self.Robot0_sent) +
                                                              tf.multiply(self.actor_params1[i], self.Robot1_sent) +
                                                              tf.multiply(self.actor_params2[i], self.Robot2_sent) +
                                                              tf.multiply(self.actor_params3[i], self.Robot3_sent) +
                                                              tf.multiply(self.actor_params4[i], self.Robot4_sent) +
                                                              tf.multiply(self.actor_params5[i], self.Robot5_sent) +
                                                              tf.multiply(self.actor_params6[i], self.Robot6_sent) +
                                                              tf.multiply(self.actor_params7[i], self.Robot7_sent) +
                                                              tf.multiply(self.actor_params8[i], self.Robot8_sent) +
                                                              tf.multiply(self.actor_params9[i], self.Robot9_sent) +
                                                              tf.multiply(self.actor_params10[i], self.Robot10_sent) +
                                                              tf.multiply(self.actor_params11[i], self.Robot11_sent) +
                                                              tf.multiply(self.actor_params12[i], self.Robot12_sent) +
                                                              tf.multiply(self.actor_params13[i], self.Robot13_sent) +
                                                              tf.multiply(self.actor_params14[i], self.Robot14_sent) +
                                                              tf.multiply(self.actor_params15[i], self.Robot15_sent) +
                                                              tf.multiply(self.actor_params16[i], self.Robot16_sent) +
                                                              tf.multiply(self.actor_params17[i], self.Robot17_sent) +
                                                              tf.multiply(self.actor_params18[i], self.Robot18_sent) +
                                                              tf.multiply(self.actor_params19[i], self.Robot19_sent)) / 20) for i in range(len(self.actor.params))]
